...
export default function Cart({ navigate }) {
  ...
  return (
    <section className="container cart-layout">
      <div>
        <div className="page-title left"><span className="eyebrow">Your bag</span><h1>Shopping cart <sup>{items.length}</sup></h1><p>Review your products before checkout.</p></div>
        <div className="shipping-progress"><div><Truck size={18} /><span>{subtotal >= 15000 ? 'You unlocked free delivery.' : `${formatCurrency(15000 - subtotal)} away from free delivery`}</span></div><progress value={Math.min(subtotal, 15000)} max="15000" /></div>
