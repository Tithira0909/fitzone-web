...
export default function ProductDetails({ slug, navigate }) {
  ...
  return (
    <>
      <div className="container breadcrumbs"><button onClick={() => navigate('/')}>Home</button><ChevronRight /><button onClick={() => navigate('/products')}>Shop</button><ChevronRight /><span>{product.name}</span></div>
      <section className="container detail-grid">
        <div className="detail-gallery">
          <div className="detail-image"><span className="detail-image-label">FIT ZONE SELECT</span><img src={activeImage} alt={product.name} /></div>
        ...
        <div className="detail-info">
          ...
          <div className="buy-row">
            ...
            <button className="btn primary detail-add" onClick={() => { addItem(product, quantity); navigate('/cart'); }} disabled={product.stock < 1}>
